...
public class OutputRCT extends Ability {
    ...
    public void run(LivingEntity owner) {
        ...
        float amount = ConfigHolder.SERVER.sorcererHealingAmount.get().floatValue() * this.getPower(owner) * 5 * 20.0F *healMult;

        if (target.getCapability(SorcererDataHandler.INSTANCE).isPresent()) {
            ISorcererData targetCap = target.getCapability(SorcererDataHandler.INSTANCE).resolve().orElseThrow();

            if (targetCap.getType() == JujutsuType.CURSE) {
                target.hurt(JJKDamageSources.jujutsuAttack(owner, this), amount * 0.86F * 1.2F);
                return;
            }
        }
        target.heal(amount);
